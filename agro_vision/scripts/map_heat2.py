...
class HeatMapGeneratorNode(Node):
    ...
    def generar_mapa(self):
        ...
        # Crear un mapa base centrado en el primer punto con tiles satelitales
        first_data = self.cacao_data[0]
        mapa = folium.Map(
            location=[first_data['latitude'], first_data['longitude']],
            zoom_start=25,  # Zoom inicial ajustado para trayectorias pequeñas
            tiles="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",  # Esri World Imagery
